# extract_air_quality_open_meteo.py
...
def write_to_bigquery(df):
    ...
    table_id = "projet-fil-orange-477313.air_quality_data.air_quality_monthly_avg"
    ...
    job.result()
    return f"Données insérées dans BigQuery : {table_id}"
...
def fetch_air_quality_data():
    ...
    variables = ["pm2_5", "pm10", "nitrogen_dioxide", "ozone", "european_aqi"]
    ...
    for country, coords in countries.items():
        ...
        for year in years:
            ...
            try:
                ...
                print(f"Erreur pour {country} en {year}: {e}")
    ...
    if all_data:
        ...
        result = write_to_bigquery(monthly_avg)
        return jsonify({"status": "success", "message": result})
    else:
        return jsonify({"status": "no_data", "message": "Aucune donnée récupérée."})
